-- Fix cast_vote_secure to properly handle data types and existing vote detection
...
BEGIN
  ...
  -- Get vote weight (simplified version)
  v_vote_weight := 1.0;
  v_weight_info := jsonb_build_object('base_weight', 1.0);
  
  -- Check for existing vote using EID/round/easel to avoid type issues
