...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    console.log('Extracted email and level:', {
      email,
      level
    });
    ...
    try {
      const { data: existingAdmin } = await supabase.from('abhq_admin_users').select('email').eq('email', email).maybeSingle();
      if (existingAdmin) {
        console.log('Admin record already exists for:', email);
        return new Response(JSON.stringify({
          error: `User ${email} already has an admin account`
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
    ...
    try {
      ...
      const { data: insertResult, error: adminCreateError } = await supabase.from('abhq_admin_users').insert(adminInsertData);
