...
serve(async (req) => {
  ...
  try {
    ...
    if (!userId || !email || !newPassword) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing userId, email, or newPassword' }),
    ...
    const { data: adminUserToUpdate, error: fetchError } = await supabase
      .from('abhq_admin_users')
    ...
    // If no auth_user_id exists, we need to create the auth user first
    if (!adminUserToUpdate.auth_user_id) {
      ...
      if (createAuthError || !authUser.user) {
        ...
        return new Response(
          JSON.stringify({ success: false, error: `Failed to create auth user: ${createAuthError?.message}` }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      // Update the abhq_admin_users record with the new auth_user_id
      ...
      if (updateError) {
        console.error('Error updating abhq_admin_users with auth_user_id:', updateError)
        // Try to cleanup the created auth user
        await supabase.auth.admin.deleteUser(authUser.user.id)
        return new Response(
          JSON.stringify({ success: false, error: `Failed to link auth user: ${updateError.message}` }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      const { error: passwordError } = await supabase.auth.admin.updateUserById(
        adminUserToUpdate.auth_user_id,
        { password: newPassword }
      )
      ...
      if (passwordError) {
        ...
        return new Response(
          JSON.stringify({ success: false, error: `Failed to update password: ${passwordError.message}` }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
