...
serve(async (req) => {
  ...
  try {
    ...
    // First, get the user's auth ID from the abhq_admin_users table
    const { data: adminUserToDelete, error: fetchError } = await supabase
      .from('abhq_admin_users')
      .select('auth_user_id')
      .eq('id', userId)
      .single()

    if (fetchError || !adminUserToDelete) {
    ...
    // If the user has an auth_user_id, also delete from Supabase Auth
    if (adminUserToDelete.auth_user_id) {
      const { error: authDeleteError } = await supabase.auth.admin.deleteUser(
        adminUserToDelete.auth_user_id
      )

      if (authDeleteError) {
        console.warn('Error deleting from Supabase Auth (user may not exist in auth):', authDeleteError)
        // Don't fail the whole operation since the admin user record is already deleted
      }
    }
