...
serve(async (req)=>{
  ...
  try {
    ...
    const { to, subject = "Art Battle Notification", html, text, from = 'hello@artbattle.com' } = await req.json();
    ...
    const createSignature = async (method, url, headers, body)=>{
      const now = new Date();
      ...
      const canonicalHeaders = Object.keys(headers).sort().map((key)=>`${key.toLowerCase()}:${headers[key]}\n`).join('');
      ...
      const getSignatureKey = async (key, dateStamp, regionName, serviceName)=>{
        ...
        const kRegion = await crypto.subtle.importKey('raw', new Uint8Array(await crypto.subtle.sign('HMAC', kDate, new TextEncoder().encode(dateStamp))), {
          name: 'HMAC',
          hash: 'SHA-256'
        }, false, [
          'sign'
        ]);
        const kService = await crypto.subtle.importKey('raw', new Uint8Array(await crypto.subtle.sign('HMAC', kRegion, new TextEncoder().encode(regionName))), {
          name: 'HMAC',
          hash: 'SHA-256'
        }, false, [
          'sign'
        ]);
        const kSigning = await crypto.subtle.importKey('raw', new Uint8Array(await crypto.subtle.sign('HMAC', kService, new TextEncoder().encode(serviceName))), {
          name: 'HMAC',
          hash: 'SHA-256'
        }, false, [
          'sign'
        ]);
        const kFinal = await crypto.subtle.importKey('raw', new Uint8Array(await crypto.subtle.sign('HMAC', kSigning, new TextEncoder().encode('aws4_request'))), {
          name: 'HMAC',
          hash: 'SHA-256'
        }, false, [
          'sign'
