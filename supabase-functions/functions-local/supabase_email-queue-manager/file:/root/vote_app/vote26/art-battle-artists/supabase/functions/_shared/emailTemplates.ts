...
artbattle.com
  ...
  paymentNotification: (data)=>({
      subject: data.soldArtworks.length > 0 ? `Art Battle ${data.cityName} - Payment Information Required ($${data.totalEarned} owed)` : `Art Battle ${data.cityName} - Thank you for participating!`,
      html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        <div style="background: #e74c3c; color: white; padding: 20px; text-align: center;">
          <h1 style="margin: 0; font-size: 24px;">🎨 Art Battle</h1>
        ...
        <div style="padding: 30px 20px;">
          <h2 style="color: #e74c3c; margin-top: 0;">
            ${data.soldArtworks.length > 0 ? `🎉 Congratulations on Your Sale${data.soldArtworks.length > 1 ? 's' : ''}!` : '🎨 Thank You for Participating!'}
          </h2>
          
          <p>Hello <strong>${data.artistName}</strong>,</p>
          
          <p>Thank you for participating in Art Battle ${data.cityName}! ${data.soldArtworks.length > 0 ? `Congratulations on the sale of your painting${data.soldArtworks.length > 1 ? 's' : ''}. We are thrilled that you were able to showcase your skills and share your art with our community.` : 'Thank you for showcasing your artistic talents and contributing to the vibrant energy of our event.'}</p>
...
Thank you for participating in Art Battle ${data.cityName}! ${data.soldArtworks.length > 0 ? `Congratulations on the sale of your painting${data.soldArtworks.length > 1 ? 's' : ''}. We are thrilled that you were able to showcase your skills and share your art with our community.` : 'Thank you for showcasing your artistic talents and contributing to the vibrant energy of our event.'}
...
${data.soldArtworks.length > 0 ? `
Your Sales Summary:
${data.soldArtworks.map((artwork)=>`- ${artwork.art_code} (Round ${artwork.round}, Easel ${artwork.easel}) - SOLD for $${artwork.sale_price} - Buyer ${artwork.payment_status}`).join('\n')}
