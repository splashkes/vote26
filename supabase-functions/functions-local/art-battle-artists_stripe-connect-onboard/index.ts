...
serve(async (req) => {
  ...
  try {
    ...
    if (!person_id || !return_url || !refresh_url) {
      console.error('Missing required parameters:', { person_id: !!person_id, return_url: !!return_url, refresh_url: !!refresh_url })
    ...
    if (isNewAccount || accountLink.url) {
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          onboarding_url_expires_at: new Date(Date.now() + (24 * 60 * 60 * 1000)).toISOString(), // 24 hours from now
