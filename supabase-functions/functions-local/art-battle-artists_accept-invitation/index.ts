...
async function sendSlackNotification(supabase: any, messageType: string, text: string) {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: existingConfirmation, error: checkError } = await supabase
      ...
      .select('id, event_eid, confirmation_status')
      .eq('artist_profile_id', submissionData.artistProfileId)
      .eq('event_eid', submissionData.eventEid)
      .maybeSingle()
    ...
    const { data: newConfirmation, error: confirmError } = await supabase
      .from('artist_confirmations')
    ...
    await sendSlackNotification(
      supabase, 
      'invitation_accepted', 
      `✅ INVITATION ACCEPTED: ${submissionData.confirmationData.legalName} confirmed for ${submissionData.eventEid} (Artist #${submissionData.artistNumber})`
    )
