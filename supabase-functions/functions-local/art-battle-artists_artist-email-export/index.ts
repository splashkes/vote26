...
serve(async (req) => {
  ...
  try {
    ...
    // Get actual bid data to determine if artworks had bids
    const { data: directBids, error: directBidsError } = await supabase
      .from('bids')
      .select(`
        art_id,
        amount,
    ...
    if (directBidsError) console.log('Direct bids fetch error:', directBidsError.message)
    ...
    // Create bid count map from actual bids table
    ...
    for (const [artistId, { artist, artworks }] of artistGroups) {
      ...
      artworks.forEach(artwork => {
        const bidCount = bidCountMap.get(artwork.id) || 0
        const highestBid = highestBidMap.get(artwork.id) || artwork.current_bid || 0
        
        if (bidCount > 0 && highestBid > 0) {
          // This artwork had bids and sold
      ...
      artworks.forEach(artwork => {
        ...
        if (bidCount > 0 && highestBid > 0) {
          ...
          detailedArtworkList += `${artworkCode} (Round ${round}, Easel ${easel}) SOLD for $${highestBid} - Buyer has ${paymentStatusText}\n`
        } else if (bidCount === 0) {
          // This artwork had no bids
