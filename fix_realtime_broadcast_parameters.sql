-- Fix realtime.broadcast_changes() call with correct parameters
-- The function expects 8 parameters including the final 'level' parameter
...
BEGIN
  ...
  -- Send broadcast using Supabase realtime.broadcast_changes() with ALL 8 parameters
  BEGIN
    PERFORM realtime.broadcast_changes(
      ...
      NEW,                                   -- new record
      OLD,                                   -- old record
      'info'                                 -- level (required 8th parameter)
    );
  EXCEPTION
    WHEN OTHERS THEN
      -- If realtime broadcast fails, don't fail the transaction
...
-- Comment explaining the correct parameter count
COMMENT ON FUNCTION broadcast_cache_invalidation IS 
'Supabase realtime broadcast cache invalidation trigger function.
CRITICAL: realtime.broadcast_changes() requires ALL 8 parameters including final level parameter.
Wrapped in try/catch to prevent trigger failures from breaking bid/vote transactions.
