-- Fix special characters in Slack notification formatting
...
BEGIN
    BEGIN
        ...
        -- Build simplified notification content with clean formatting
        IF artist_info.bio IS NOT NULL AND LENGTH(TRIM(artist_info.bio)) > 0 THEN
            bio_preview := LEFT(TRIM(artist_info.bio), 100);
            IF LENGTH(artist_info.bio) > 100 THEN
                bio_preview := bio_preview || '...';
            END IF;
        ELSE
            bio_preview := '_No artist statement provided_';
        END IF;

        -- Header format: "ARTIST NAME applied to ABXXXX on DATE"
        ...
        -- Clean main text format without special characters
