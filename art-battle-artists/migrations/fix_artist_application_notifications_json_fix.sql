-- Fix the JSON/JSONB type mismatch in artist application notifications
...
BEGIN
    BEGIN
        ...
        -- Get sample works using the unified function - with error handling
        sample_works := '[]'::jsonb; -- Default to empty array
        BEGIN
            SELECT array_to_json(array_agg(row_to_json(works)))::jsonb
            INTO sample_works
            FROM (
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'Error getting sample works for profile % (continuing with empty): %', NEW.artist_profile_id, SQLERRM;
                sample_works := '[]'::jsonb;
        END;
        ...
        -- Create blocks with sample works if available - USING JSONB consistently
        ...
        ELSE
            ...
            slack_blocks := slack_blocks || jsonb_build_array(
                ...
                jsonb_build_object(
                    ...
                    'text', jsonb_build_object(
                        'type', 'mrkdwn',
                        'text', '_No sample works available in portfolio_'
                    )
                )
            );
        END IF;
