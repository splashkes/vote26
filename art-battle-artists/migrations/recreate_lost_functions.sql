-- Recreate lost database functions and schema changes
-- Run with: PGPASSWORD='6kEtvU9n0KhTVr5' psql -h db.xsqdkubgyqwpyvfltnrf.supabase.co -p 5432 -d postgres -U postgres -f migrations/recreate_lost_functions.sql

-- 1. Add aliases column to artist_profiles
ALTER TABLE artist_profiles ADD COLUMN IF NOT EXISTS aliases JSONB DEFAULT '[]'::jsonb;
CREATE INDEX IF NOT EXISTS idx_artist_profiles_aliases ON artist_profiles USING GIN (aliases);

-- 2. Create artist ID generator function
...
-- 3. Create multi-profile lookup function
