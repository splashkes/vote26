-- Fix RLS policies for artist_sample_works table
...
-- Check if the artist_profile belongs to the authenticated user's person
CREATE POLICY "Artists can manage own sample works" ON artist_sample_works
    ...
    USING (
        artist_profile_id IN (
            ...
            WHERE person_id = (auth.jwt() -> 'app_metadata' ->> 'person_id')::uuid
        )
    )
    WITH CHECK (
        artist_profile_id IN (
