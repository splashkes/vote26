...
serve(async (req)=>{
  ...
  try {
    ...
    // Send Slack notification for successful profile update
    await sendSlackNotification(supabase, 'profile_update_success', `✅ Profile Updated Successfully\nArtist: ${updatedProfile.name}\nProfile ID: ${profileId}\nUser: ${authUserId || 'unknown'}\nPerson: ${personId}\nUpdated Fields: ${Object.keys(updateData).filter((key)=>updateData[key] !== null).join(', ')}\nDuration: ${duration}ms\nIP: ${ipAddress}`);
