-- Add votes and bids as sources for SMS audience targeting
-- This expands SMS marketing to include everyone who has interacted with events
-- Not just registrations and QR scans, but also votes and bids
    ...
    WHERE
      CASE
        ...
        WHEN p_city_ids IS NOT NULL AND p_event_ids IS NOT NULL THEN
          p.id IN (
            -- Get people from registrations, QR scans, votes, AND bids
