  ...
  try {
    ...
    if (method === 'GET') {
      if (action === 'list') {
        ...
        if (!eventEid) {
          return new Response(JSON.stringify({
            error: 'event_eid parameter required'
          }), {
            status: 400,
            headers: {
              ...corsHeaders,
        ...
        const { data: queueEntries, error } = await supabase.from('artist_payment_email_queue').select(`
            *,
            events(eid, name, cities(name)),
            artist_profiles(name, entry_id, person:people(email))
          `).eq('events.eid', eventEid).order('created_at', {
          ascending: false
        });
        if (error) {
          return new Response(JSON.stringify({
      ...
      if (action === 'preview') {
        ...
        if (!emailId) {
          return new Response(JSON.stringify({
            error: 'email_id parameter required'
          }), {
            status: 400,
            headers: {
              ...corsHeaders,
        ...
        const { data: emailEntry, error } = await supabase.from('artist_payment_email_queue').select(`
            *,
            events(eid, name, cities(name)),
            artist_profiles(name, entry_id, person:people(email))
      ...
      if (action === 'stats') {
        ...
        const { data: eventData, error: eventError } = await supabase.from('events').select('id').eq('eid', eventEid).single();
        if (eventError) {
          return new Response(JSON.stringify({
        ...
        const { data: queueEntries, error: queueError } = await supabase.from('artist_payment_email_queue').select('status').eq('event_id', eventData.id);
        if (queueError) {
          return new Response(JSON.stringify({
    ...
    if (method === 'PUT') {
      if (action === 'approve') {
        ...
        if (!email_ids || !Array.isArray(email_ids) || email_ids.length === 0) {
          return new Response(JSON.stringify({
            error: 'email_ids array required'
          }), {
            status: 400,
            headers: {
              ...corsHeaders,
        ...
        const { data, error } = await supabase.from('artist_payment_email_queue').update({
          status: 'approved',
          approved_by: adminUser.id,
          approved_at: new Date().toISOString()
        }).in('id', email_ids).select();
        if (error) {
          return new Response(JSON.stringify({
            error: error.message
          }), {
      ...
      if (action === 'update_status') {
        ...
        if (!email_id || !status) {
          return new Response(JSON.stringify({
            error: 'email_id and status required'
          }), {
            status: 400,
            headers: {
              ...corsHeaders,
        ...
        const { data, error } = await supabase.from('artist_payment_email_queue').update(updateData).eq('id', email_id).select().single();
        if (error) {
          return new Response(JSON.stringify({
            error: error.message
          }), {
    ...
    if (method === 'POST') {
      if (action === 'send') {
        ...
        const { data: emailsToSend, error: fetchError } = await supabase.from('artist_payment_email_queue').select(`
            *,
            artist_profiles(name, person:people(email))
          `).in('id', email_ids).eq('status', 'approved');
        if (fetchError) {
          return new Response(JSON.stringify({
        ...
        for (const emailEntry of emailsToSend){
          try {
            ...
            if (emailResult.success) {
              ...
              await supabase.from('artist_payment_email_queue').update({
                status: 'sent',
