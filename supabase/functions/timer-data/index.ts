...
serve(async (req) => {
  ...
  try {
    ...
    const { data: event, error: eventError } = await supabase
      .from('events')
      .select('id, eid, name, venue, current_round, event_start_datetime')
      .eq('eid', eid)
      .single()
    ...
    // Get city from venue or extract from name
    const eventName = event.name || ''
    const eventParts = eventName.split(' - ')
    const city = eventParts.length > 1 ? eventParts[1] : 'Unknown'
