...
serve(async (req) => {
  ...
  try {
    ...
    // Get the highest bid amount and the person who placed it
    ...
    if (bidsError || !allBids || allBids.length === 0) {
      throw new Error('No bids found for this artwork')
    }
    
    // Find the highest bid amount
    const maxAmount = Math.max(...allBids.map(bid => bid.amount))
    const winningBid = allBids.find(bid => bid.amount === maxAmount)
    ...
    // TODO: Consider adding cleanup of old pending/failed payments to prevent stale session issues
    // This would prevent cases where old payment_processing records with wrong amounts
    // interfere with new payment attempts. Implementation should:
    // 1. Delete payment_processing records with status 'pending' or 'failed' for this art_id
    // 2. Be careful not to delete legitimate in-progress payments
    // 3. Consider adding a created_at timestamp check (e.g., older than 1 hour)
    // Example:
    // await supabase.from('payment_processing').delete()
    //   .eq('art_id', art_id).in('status', ['pending', 'failed'])
