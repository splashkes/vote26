...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: success_url || `${Deno.env.get('SITE_URL')}/events/${event.id}?payment=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: cancel_url || `${Deno.env.get('SITE_URL')}/events/${event.id}?payment=cancelled`,
