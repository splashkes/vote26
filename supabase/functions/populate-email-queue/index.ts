  ...
  try {
    ...
    if (!eventEid || eventEid === 'populate-email-queue') {
      return new Response(JSON.stringify({
        error: 'Event EID is required in URL path (e.g., /populate-email-queue/AB3019)'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: artworks, error: artError } = await supabase.from('art').select(`
      ...
      `).eq('event_id', event.id).not('artist_profiles.entry_id', 'is', null).order('artist_id, art_code');
    if (artError) {
    ...
    const { data: paymentData, error: paymentError } = await supabase.rpc('get_payment_logs_admin', {
      p_event_id: event.id
    });
    if (paymentError) console.log('Payment data fetch error:', paymentError.message);
    const { data: paymentStatuses, error: paymentStatusError } = await supabase.rpc('get_payment_statuses_admin', {
      p_event_id: event.id
    });
    if (paymentStatusError) console.log('Payment status fetch error:', paymentStatusError.message);
    const { data: stripePayments, error: stripeError } = await supabase.from('payment_processing').select(`
        art_id,
        status,
        metadata,
        art!inner(event_id)
    ...
    for (const [artistId, { artist, artworks }] of artistGroups){
      ...
      const templateData = {
        ...
        paymentMethodText: cityName.toLowerCase().includes('toronto') || cityName.toLowerCase().includes('montreal') ? 'We send payments via Interac e-Transfer or PayPal. Please confirm one of the following, so we may get your payment sent promptly\n\nInterac e-Transfer - email address\n\nPayPal - email or handle' : 'We exclusively send payments via PayPal or Zelle. Please confirm one of the following, so we may get your payment sent promptly\n\nPayPal - email or handle\n\nZelle - email or phone',
        hasUnpaidSales: hasUnpaidSales,
        artistEmail: artistEmail
      };
      try {
        // Insert into queue using UPSERT to handle duplicates
        const { data: queueEntry, error: insertError } = await supabase.from('artist_payment_email_queue').upsert({
          event_id: event.id,
