...
serve(async (req) => {
  ...
  try {
    ...
    if (personIdFromMeta) {
      ...
      const { data: qrPersonData } = await supabase
        .from('people')
        .select('hash, name')
        .eq('id', personId)
        .single()
      ...
      if (!qrPersonHash) {
        ...
        await supabase
          .from('people')
          .update({ hash: qrPersonHash })
          .eq('id', personId)
      }

      // Update auth user metadata with person info (fire-and-forget)
      updateAuthUserMetadata(newRecord.id, personId, qrPersonHash, qrPersonData?.name || personName || 'User').catch(err => 
        console.warn('Auth metadata update failed (non-critical):', err)
      )

      // Send success notification (fire-and-forget)
      sendPersonLinkNotification(newRecord.id, personId, qrPersonData?.name || personName || 'User', authPhone, 'linked_qr').catch(err =>
      ...
      if (existingPersonByPhone) {
        ...
        // Update auth user metadata with person info (fire-and-forget)
        updateAuthUserMetadata(newRecord.id, personId, personHash, existingPersonByPhone.name || 'User').catch(err => 
          console.warn('Auth metadata update failed (non-critical):', err)
        )

        // Send success notification (fire-and-forget)
        sendPersonLinkNotification(newRecord.id, personId, existingPersonByPhone.name || 'User', authPhone, 'linked_existing').catch(err =>
          console.warn('Slack notification failed (non-critical):', err)
        )
        ...
        // Update auth user metadata with person info (fire-and-forget)
        updateAuthUserMetadata(newRecord.id, personId, personHash, 'User').catch(err => 
          console.warn('Auth metadata update failed (non-critical):', err)
        )

        // Send success notification (fire-and-forget)
        sendPersonLinkNotification(newRecord.id, personId, 'User', authPhone, 'created_new').catch(err =>
          console.warn('Slack notification failed (non-critical):', err)
        )
...
async function generatePersonHash(personId: string, phone: string): Promise<string> {
...
async function updateAuthUserMetadata(userId: string, personId: string, personHash: string, personName: string) {
  try {
...
async function sendPersonLinkNotification(userId: string, personId: string, personName: string, phone: string, action: string) {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
...
async function logAuthActivity(userId: string, personId: string, phone: string, action: string, details: any) {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
