-- Enhanced broadcast trigger with per-endpoint cache version updates
-- This updates specific endpoint timestamps for surgical cache invalidation
...
-- Comment explaining the enhanced approach
...
Updates specific endpoint timestamps in endpoint_cache_versions table for surgical cache-busting.
Only endpoints that actually changed get new cache versions, maintaining CDN efficiency.';

-- Also update the /live/events endpoint when events table changes
CREATE OR REPLACE FUNCTION broadcast_events_cache_invalidation()
RETURNS TRIGGER AS $$
DECLARE
  v_cache_version BIGINT;
BEGIN
...
DROP TRIGGER IF EXISTS trigger_events_cache_invalidation ON events;
CREATE TRIGGER trigger_events_cache_invalidation
  AFTER INSERT OR UPDATE OR DELETE ON events
  FOR EACH ROW EXECUTE FUNCTION broadcast_events_cache_invalidation();