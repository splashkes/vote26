-- Fix the INSERT statement in process_bid_secure to match actual bids table columns
-- Remove mongo references and fix column mismatches
...
BEGIN
  ...
  -- FIXED INSERT: Match actual bids table columns exactly
  INSERT INTO bids (
    id,
    art_id,        -- Use v_art_uuid (UUID), not p_art_id (text)
    person_id,
    amount,
    created_at,
    currency_code,
  ...
  -- Try to check auction extension (this might fail)
  BEGIN
  ...
  -- Try to queue notification (this might fail)
  BEGIN
