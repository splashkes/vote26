-- Fix vote weight system UUID comparison errors
-- The issue is that vote functions are using art_id (VARCHAR) instead of art_uuid (UUID)

-- Drop the existing function to avoid conflicts
DROP FUNCTION IF EXISTS cast_vote_secure(VARCHAR, INT, INT);
DROP FUNCTION IF EXISTS cast_vote_secure(TEXT);

-- Create corrected vote function with full weight system using proper UUID columns
...
'Cast vote with full weight calculation including QR bonus.
CRITICAL FIX: Uses art_uuid (UUID) for vote lookups instead of art_id (VARCHAR) 
to prevent "operator does not exist: uuid = character varying" errors.
...
- Stores both art_id (VARCHAR) and art_uuid (UUID) for compatibility';