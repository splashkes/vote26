-- Fix broadcast system using realtime.send() from the Supabase blog
...
BEGIN
  ...
  -- Send broadcast using realtime.send() as per Supabase blog
  BEGIN
    PERFORM realtime.send(
      'cache_invalidate_' || v_event_eid,    -- channel
      'cache_invalidation',                   -- event
      v_notification_payload                  -- payload
    );
...
'Broadcast cache invalidation using realtime.send() from Supabase blog.
Uses realtime.send(channel, event, payload) to send broadcasts from database triggers.
Clients can listen to these broadcasts using supabase.channel().on("broadcast", ...)
