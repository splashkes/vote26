-- Create a minimal test function to isolate the UUID comparison error
CREATE OR REPLACE FUNCTION test_vote_debug(
  p_eid VARCHAR(20),
  p_round INT,
  p_easel INT
) RETURNS JSONB AS $$
...
GRANT EXECUTE ON FUNCTION test_vote_debug(VARCHAR, INT, INT) TO authenticated;