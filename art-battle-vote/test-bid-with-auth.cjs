...
async function testBidWithAuth() {
  ...
  try {
    ...
    await page.screenshot({ path: 'page-after-auth.png' });
    console.log('Screenshot saved as page-after-auth.png');
    
    // Wait for events to load - using a more generic selector
    console.log('Waiting for content to load...');
    await page.waitForSelector('div[role="region"]', { timeout: 10000 });
    
    // Find and click on an event - look for event cards more flexibly
    console.log('Looking for events...');
    const eventCards = await page.$$('div[role="region"] > div');
    if (eventCards.length > 0) {
      console.log(`Found ${eventCards.length} event cards`);
      await eventCards[0].click();
      console.log('Clicked on first event');
    } else {
      console.log('No event cards found, checking page content...');
