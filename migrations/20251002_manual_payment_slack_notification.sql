-- Migration: Manual Payment Request Slack Notifications
-- Purpose: Send Slack notification when artist submits manual payment request
-- Date: 2025-10-02

-- Create trigger function for manual payment request notifications
CREATE OR REPLACE FUNCTION notify_manual_payment_request_slack()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Create trigger on artist_manual_payment_requests table
DROP TRIGGER IF EXISTS trigger_manual_payment_request_slack ON artist_manual_payment_requests;

CREATE TRIGGER trigger_manual_payment_request_slack
    AFTER INSERT ON artist_manual_payment_requests
    FOR EACH ROW
    EXECUTE FUNCTION notify_manual_payment_request_slack();

COMMENT ON FUNCTION notify_manual_payment_request_slack() IS 'Sends Slack notification when artist submits manual payment request';
COMMENT ON TRIGGER trigger_manual_payment_request_slack ON artist_manual_payment_requests IS 'Triggers Slack notification on new manual payment request';
