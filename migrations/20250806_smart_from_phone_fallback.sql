-- Update send_sms_instantly to use event phone with fallback to hardcoded phone
...
BEGIN
  ...
  -- Final safety check - if still same as destination, use fallback
  IF v_from_phone = p_destination THEN
    v_from_phone := v_hardcoded_fallback;
  END IF;
  ...
  RAISE NOTICE 'SMS queued - To: %, From: %, Event ID: %', p_destination, v_from_phone, v_event_id;
  
  RETURN v_message_id;
END;
$$;

-- Also update send_auction_closing_notifications to use smart fallback
