-- Update payment calculation functions to use dynamic artist_auction_portion
-- Drop and recreate functions that have signature conflicts

-- Drop existing functions that need updates
DROP FUNCTION IF EXISTS get_event_payment_summary(UUID);
DROP FUNCTION IF EXISTS get_artists_owed();

-- 1. Update get_event_payment_summary function (recreate with dynamic percentage)
...
-- 2. Update global get_artists_owed function (recreate with dynamic percentage)
...
SELECT 'Payment functions updated with dynamic percentages' as status;