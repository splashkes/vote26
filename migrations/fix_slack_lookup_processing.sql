-- Fix Slack lookup processing by updating the batch processor to handle pending_lookup notifications
-- This migration modifies process_slack_queue_batch to also process channel lookups
...
COMMENT ON FUNCTION process_slack_queue_batch(integer) IS 
'Enhanced batch processor that handles both channel lookups and regular notification processing.
First processes pending_lookup notifications to resolve channel IDs, then processes regular pending notifications.';