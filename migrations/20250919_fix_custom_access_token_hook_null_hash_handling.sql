-- Fix custom_access_token_hook to generate hash if NULL is found
-- This prevents authentication failures when existing users have NULL hash values
...
BEGIN
    ...
    IF FOUND THEN
        -- CRITICAL FIX: Generate hash if NULL to prevent authentication failures
        IF person_data.hash IS NULL THEN
            generated_hash := substring(md5(random()::text) from 1 for 8);

            UPDATE public.people
            SET hash = generated_hash,
                updated_at = now()
            WHERE id = person_data.id;

            person_data.hash := generated_hash;
        END IF;
    ...
    ELSE
        ...
        IF user_phone IS NOT NULL AND user_phone != '' THEN
            ...
            IF FOUND THEN
                -- CRITICAL FIX: Generate hash if NULL for existing person being linked
                IF person_data.hash IS NULL THEN
                    generated_hash := substring(md5(random()::text) from 1 for 8);
                    person_data.hash := generated_hash;
                ELSE
                    generated_hash := person_data.hash;
                END IF;
                ...
                SET auth_user_id = user_id::uuid,
                    ...
                    hash = generated_hash,  -- Ensure hash is set
                    updated_at = now()
                WHERE id = person_data.id;

                claims := jsonb_set(claims, '{person_id}', to_jsonb(person_data.id::text));
                claims := jsonb_set(claims, '{person_hash}', to_jsonb(generated_hash));
                claims := jsonb_set(claims, '{person_name}', to_jsonb(person_data.name));
                claims := jsonb_set(claims, '{person_verified}', to_jsonb(person_data.verified));
                claims := jsonb_set(claims, '{auth_version}', '"v2-http"');
                claims := jsonb_set(claims, '{person_pending}', 'false');
    ...
    -- FINAL FALLBACK: If claims are invalid/error, create new person and unlink old one
    -- This ensures users can ALWAYS authenticate even with data corruption
    IF claims IS NULL OR
       NOT claims ? 'person_hash' OR
       claims->>'person_hash' IS NULL OR
       claims->>'person_hash' = '' THEN

        -- Only proceed if we have a phone number
        IF user_phone IS NOT NULL AND user_phone != '' THEN
            -- Unlink any existing person from this auth user
            UPDATE public.people
            ...
            -- Create entirely new person record
            generated_hash := substring(md5(random()::text) from 1 for 8);
            ...
            -- Return fresh claims for new person
            claims := '{}'::jsonb;
            claims := jsonb_set(claims, '{person_id}', to_jsonb(new_person_id));
            claims := jsonb_set(claims, '{person_hash}', to_jsonb(generated_hash));
            claims := jsonb_set(claims, '{person_name}', '"User"');
            ...
            -- Re-add admin permissions for new person
            IF user_phone IS NOT NULL AND user_phone != '' THEN
                normalized_phone := regexp_replace(user_phone, '^\+', '', 'g');
                admin_events := '{}';
