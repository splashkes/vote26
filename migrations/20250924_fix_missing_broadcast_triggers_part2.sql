-- Fix Missing Broadcast Triggers Part 2
-- Date: 2025-09-24
-- Issues: Missing UPDATE trigger on bids table, art_media not broadcasting to main endpoint

-- 1. Add missing UPDATE trigger for bids table
-- This is critical - when bid amounts are updated, users need to see the changes
CREATE OR REPLACE TRIGGER cache_invalidate_bids_update_trigger
    AFTER UPDATE ON bids
    FOR EACH ROW
    EXECUTE FUNCTION broadcast_cache_invalidation();

-- 2. Modify broadcast function to also invalidate main event endpoint for art_media
-- This ensures photo uploads show up in Vote section, not just media endpoint
