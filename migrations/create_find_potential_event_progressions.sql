-- Create function to find potential event progression relationships
-- This identifies artists who won events (most votes in R3) and then competed in another event within 1 year

CREATE OR REPLACE FUNCTION find_potential_event_progressions()
RETURNS TABLE (
  artist_name varchar,
  artist_number varchar,
  first_event_eid varchar,
  first_event_name varchar,
  first_event_date timestamp,
  first_event_city varchar,
  r3_votes bigint,
  next_event_eid varchar,
  next_event_name varchar,
  next_event_date timestamp,
  next_event_city varchar,
...
-- Add comment
COMMENT ON FUNCTION find_potential_event_progressions IS 'Identifies potential event progression relationships based on artists who won (most R3 votes) and then competed in another event within 1 year';
