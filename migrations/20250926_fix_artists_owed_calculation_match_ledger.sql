-- Fix get_enhanced_admin_artists_owed function to match ledger calculation
-- This ensures that 'amount owed' matches the ledger by including ALL payment statuses as debits
...
AS $function$
  ...
  payment_debits AS (
    SELECT
      ...
      -- This includes: pending, processing, paid, verified, failed, cancelled
      SUM(ap.gross_amount) as debits_total
    FROM artist_payments ap
    -- No status filter - include ALL payments like the ledger does
  ...
  SELECT
    ...
    -- Updated calculation to match ledger: credits - ALL debits (not just completed/paid)
...
VALUES (
    'migration',
    'fix_artists_owed_calculation',
    'info',
    'Updated get_enhanced_admin_artists_owed function to match ledger calculation by including ALL payment statuses',
    jsonb_build_object(
        'migration_file', '20250926_fix_artists_owed_calculation_match_ledger.sql',
        'applied_at', NOW()::text,
        'change_description', 'Removed status filter from payment_debits CTE to include all payments like ledger does',
        'previous_filter', 'status IN (completed, paid)',
        'new_filter', 'no status filter - includes all payments'
    )
);