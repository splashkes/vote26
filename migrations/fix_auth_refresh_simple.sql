-- Simple fix for refresh_auth_metadata function - remove explicit casting in exception handler
-- Date: August 26, 2025
-- Issue: Function was working before, just remove unnecessary explicit casting
...
BEGIN
  ...
  IF v_person_id IS NOT NULL THEN
    ...
    IF v_person_hash IS NULL THEN
      -- Fix: Cast both parameters to text explicitly (this was the original digest issue)
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    -- Log error WITHOUT explicit casting - this is how it worked before
    PERFORM log_artist_auth(
      v_auth_user_id, v_person_id, v_auth_phone,
      'metadata_refresh', 'refresh_auth_metadata',
