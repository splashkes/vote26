-- Remove raw_user_meta_data dependencies from process_bid_secure
...
BEGIN
  ...
  WHERE LOWER(a.art_id) = LOWER(p_art_id);
...
-- Add comment
COMMENT ON FUNCTION process_bid_secure(text, numeric) IS 'Process bid with auth-first approach - no metadata dependencies';

SELECT 'process_bid_secure updated - removed raw_user_meta_data dependencies' AS status;