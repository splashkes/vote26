-- Fix refresh_auth_metadata function - ensure proper parameter types in exception handler
-- Date: August 26, 2025
-- Issue: log_artist_auth function signature mismatch in exception handler
...
DECLARE
  ...
  v_duration_ms INTEGER := 0; -- Initialize to prevent NULL issues
...
EXCEPTION
  WHEN OTHERS THEN
    -- Calculate duration even in exception
    v_duration_ms := COALESCE(EXTRACT(EPOCH FROM (clock_timestamp() - v_start_time)) * 1000, 0)::INTEGER;
    
    -- Log error with explicit type casting to prevent signature mismatch
    PERFORM log_artist_auth(
      v_auth_user_id::UUID,
      v_person_id::UUID, 
      v_auth_phone::TEXT,
      'metadata_refresh'::TEXT, 
      'refresh_auth_metadata'::TEXT,
      false::BOOLEAN, 
      'database_error'::TEXT, 
      SQLERRM::TEXT, 
      v_duration_ms::INTEGER,
      jsonb_build_object('sql_error', SQLERRM, 'sql_state', SQLSTATE)::JSONB
    );

    -- Slack notification for database error
    PERFORM queue_slack_notification(
      'profile-debug',
      'auth_refresh_failed',
      ...
      'Error: ' || SQLERRM || E'\n' ||
      'Duration: ' || v_duration_ms::text || 'ms'
    );
