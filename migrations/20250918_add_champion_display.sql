-- Add champion/winner detection to timer data when all rounds complete
...
BEGIN
    ...
    -- Get champion data (winner from highest round with a single winner)
    WITH round_winners AS (
        SELECT
            r.round_number,
            r.closing_time,
            ap.name as winner_name,
            ROW_NUMBER() OVER (PARTITION BY r.id ORDER BY rv.vote_count DESC) as rank,
            COUNT(*) OVER (PARTITION BY r.id) as total_contestants
        FROM rounds r
        ...
        LEFT JOIN (
            ...
            FROM round_contestants rc2
            LEFT JOIN votes v ON rc2.round_id = v.round_id AND rc2.artist_id = v.artist_id
            GROUP BY rc2.round_id, rc2.artist_id
        ) rv ON r.id = rv.round_id AND rc.artist_id = rv.artist_id
        WHERE r.event_id = event_record.id
          AND r.closing_time IS NOT NULL
          AND r.closing_time < now()
    ),
    highest_round_winner AS (
        SELECT
            winner_name,
            round_number
        FROM round_winners
        WHERE rank = 1
          AND total_contestants > 1
        ORDER BY round_number DESC
        LIMIT 1
    )
    SELECT CASE
        WHEN EXISTS (SELECT 1 FROM highest_round_winner) THEN
            jsonb_build_object(
                'has_champion', true,
                'champion_name', (SELECT winner_name FROM highest_round_winner),
                'winning_round', (SELECT round_number FROM highest_round_winner)
            )
        ELSE
...
'Enhanced timer data function with champion detection for post-event display';