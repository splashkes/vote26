-- ADD PAYMENT COMPLETION CACHE INVALIDATION
-- Extends broadcast_cache_invalidation function to handle payment_processing table
-- Ensures immediate UI updates when payments complete
-- Date: 2025-09-18

-- ============================================
-- Extend broadcast_cache_invalidation function to handle payment_processing
-- ============================================
...
-- ============================================
-- Create trigger for payment_processing table
-- ============================================

-- Drop existing trigger if it exists
DROP TRIGGER IF EXISTS cache_invalidate_payment_processing_trigger ON payment_processing;

-- Create trigger for payment status changes (only on UPDATE to avoid noise on INSERT)
CREATE TRIGGER cache_invalidate_payment_processing_trigger
...
DO $$
BEGIN
  RAISE NOTICE '✅ Extended broadcast_cache_invalidation function for payment_processing';
  RAISE NOTICE '✅ Added cache invalidation trigger for payment status changes';
  RAISE NOTICE '';
  RAISE NOTICE 'Payment Cache Invalidation Ready:';
  RAISE NOTICE '- Payment completion triggers immediate event data refresh';
  RAISE NOTICE '- All users (admin and guests) see payment status updates instantly';
  RAISE NOTICE '- Only triggers on status changes to completed/failed (not on INSERT)';
  RAISE NOTICE '- Safe fallback handling prevents trigger failures';
END $$;