-- Profile Update Slack Notifications
-- Adds trigger to send Slack notifications when artist profiles are updated
-- Includes phone numbers and email addresses from artist_profiles and people tables

-- Function to send profile update Slack notification
CREATE OR REPLACE FUNCTION notify_profile_update_slack()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trigger_profile_update_slack ON artist_profiles;

CREATE TRIGGER trigger_profile_update_slack
    AFTER INSERT OR UPDATE ON artist_profiles
    FOR EACH ROW
    EXECUTE FUNCTION notify_profile_update_slack();

-- Add comment
COMMENT ON FUNCTION notify_profile_update_slack() IS 'Sends Slack notifications when artist profiles are updated, including contact information from both artist_profiles and people tables';
COMMENT ON TRIGGER trigger_profile_update_slack ON artist_profiles IS 'Triggers Slack notifications for profile updates with contact information';

SELECT 'Profile update Slack notification system added successfully' AS status;