-- Add auction value tracking to time series function
-- This calculates the sum of highest bids per artwork at each time interval

DROP FUNCTION IF EXISTS public.get_event_time_series(uuid, integer);

CREATE FUNCTION public.get_event_time_series(p_event_id uuid, p_interval_minutes integer DEFAULT 30)
RETURNS TABLE(
    ...
    auction_value_cumulative numeric,
    auction_value_interval numeric
)
LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- Get bids data with auction value calculation
    bid_data AS (
        SELECT
            b.created_at,
            b.art_id,
    ...
    interval_stats AS (
        SELECT
            ...
            -- Auction value calculations - sum of highest bids per artwork
            (SELECT COALESCE(SUM(highest_bid), 0)
             FROM (
                 SELECT MAX(bd.amount) as highest_bid
                 FROM bid_data bd
                 WHERE bd.created_at <= ti.interval_start + (p_interval_minutes || ' minutes')::interval
                 GROUP BY bd.art_id
             ) highest_bids_cumulative
            ) as auction_value_cumulative,
            (SELECT COALESCE(SUM(highest_bid), 0)
             FROM (
                 SELECT MAX(bd.amount) as highest_bid
                 FROM bid_data bd
                 WHERE bd.created_at BETWEEN ti.interval_start AND ti.interval_start + (p_interval_minutes || ' minutes')::interval
                 GROUP BY bd.art_id
             ) highest_bids_interval
            ) as auction_value_interval
    ...
    SELECT
        ...
        iss.auction_value_cumulative::numeric,
        iss.auction_value_interval::numeric
