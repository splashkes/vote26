-- Fix manual payment currency handling in event-specific artists owed calculation

-- Drop existing function first
DROP FUNCTION IF EXISTS public.get_event_artists_owed(UUID);
...
VALUES (
    'migration',
    'fix_event_manual_payment_currency_handling',
    'info',
    'Fixed event-specific manual payment currency handling to properly offset earnings across currencies',
    jsonb_build_object(
        'migration_file', '20250926_fix_event_manual_payment_currency_handling.sql',
        'applied_at', NOW()::text,
        'change_description', 'Event-scoped manual payments now offset earnings regardless of currency mismatch',
        'updated_function', 'get_event_artists_owed'
    )
);