-- Fix get_event_artists_owed function to use dynamic artist_auction_portion
-- The previous migration didn't update this function correctly
...
BEGIN
  ...
  WITH event_art_sales AS (
    SELECT
      ...
      SUM(COALESCE(a.final_price, a.current_bid, 0) * e.artist_auction_portion) as sales_total -- FIXED: Use dynamic percentage
    FROM art a
  ...
  event_automated_payment_debits AS (
    -- Only automated payments for this event (should match currency)
    SELECT
  ...
  event_manual_payment_totals AS (
    -- Manual payments for this event: treat as offsetting regardless of currency
    SELECT
      ap.artist_profile_id,
  ...
  event_artist_balances AS (
    ...
    LEFT JOIN event_art_sales sales ON ap.id = sales.artist_id
    ...
    WHERE sales.artist_id IS NOT NULL  -- Only artists with sales in this event
  ),
  ...
  SELECT
    ...
    e.name::TEXT,
    1::INTEGER, -- They participated in this event
...
COMMENT ON FUNCTION get_event_artists_owed(UUID) IS 'FIXED: Now uses dynamic artist_auction_portion from events table instead of hardcoded 50%';

SELECT 'get_event_artists_owed function fixed with dynamic percentage' as status;