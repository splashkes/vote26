-- FINAL FIX for refresh_auth_metadata function - explicit text casting for string literals
-- Date: August 26, 2025
-- Issue: String literals in function calls need explicit casting due to plpgsql context
...
BEGIN
  ...
  IF v_auth_user_id IS NULL THEN
    PERFORM log_artist_auth(
      NULL, NULL, NULL,
      'metadata_refresh'::TEXT, 'refresh_auth_metadata'::TEXT,
      false, 'not_authenticated'::TEXT, 'No authenticated user found'::TEXT
    );

    PERFORM queue_slack_notification(
      'profile-debug'::TEXT,
      'auth_refresh_failed'::TEXT,
      '🚫 Auth Refresh Failed: No authenticated user found'::TEXT
    );
  ...
  IF v_auth_phone IS NULL THEN
    PERFORM log_artist_auth(
      ...
      'metadata_refresh'::TEXT, 'refresh_auth_metadata'::TEXT,
      false, 'phone_missing'::TEXT, 'No phone number found in auth record'::TEXT
    );

    PERFORM queue_slack_notification(
      'profile-debug'::TEXT,
      'auth_refresh_failed'::TEXT,
      ('📞 Auth Refresh Failed: No phone number found' || E'\nUser: ' || v_auth_user_id::text)::TEXT
    );
  ...
  ELSE
    ...
    INSERT INTO people (
      id, phone, name, nickname, hash, auth_user_id, auth_phone, verified, created_at, updated_at
    ) VALUES (
      v_person_id, '+1' || v_normalized_phone, v_person_name, v_person_name, v_person_hash,
      v_auth_user_id, v_auth_phone, true, NOW(), NOW()
    );
...
EXCEPTION
  WHEN OTHERS THEN
    v_duration_ms := COALESCE(EXTRACT(EPOCH FROM (clock_timestamp() - v_start_time)) * 1000, 0);
    
    -- Fixed: Explicit casting for all parameters in exception handler
    PERFORM log_artist_auth(
      v_auth_user_id, v_person_id, v_auth_phone,
      'metadata_refresh'::TEXT, 'refresh_auth_metadata'::TEXT,
      false, 'database_error'::TEXT, SQLERRM::TEXT, v_duration_ms,
