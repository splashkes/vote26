-- Fix Artist Currency in Admin Payments
-- Add currency_code and currency_symbol to get_simple_admin_payments_data function
-- This ensures artists are paid in their local currency (e.g., THB for Thailand, not USD)

DROP FUNCTION IF EXISTS public.get_simple_admin_payments_data(integer);

CREATE OR REPLACE FUNCTION public.get_simple_admin_payments_data(days_back integer)
RETURNS TABLE(
  artist_id uuid,
  artist_name text,
  artist_email text,
...
BEGIN
  ...
  SELECT
    ...
    (rc.contest_count IS NOT NULL AND rc.contest_count > 0)::BOOLEAN,
    ...
    COALESCE(co.currency_code, 'USD')::TEXT,
    COALESCE(co.currency_symbol, '$')::TEXT
  FROM artist_profiles ap
  LEFT JOIN artist_global_payments agp ON ap.id = agp.artist_profile_id
  LEFT JOIN countries co ON ap.country = co.code
...
GRANT EXECUTE ON FUNCTION public.get_simple_admin_payments_data(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_simple_admin_payments_data(integer) TO service_role;
