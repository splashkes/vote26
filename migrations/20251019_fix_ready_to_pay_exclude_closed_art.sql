-- Fix get_ready_to_pay_artists to exclude 'closed' art status
-- ISSUE: Gabriel Antonio (entry_id: 310249) showing $50 balance in "ready to pay" but $0 in ledger
-- ROOT CAUSE: art_sales_by_currency CTE includes 'closed' status
--             but ledger only counts 'sold' and 'paid' as credits
-- CLOSED = auction ended with no sale, artist gets $0
...
BEGIN
  ...
  WITH art_sales_by_currency AS (
    ...
    -- FIXED: Only count 'sold' and 'paid' to match ledger
    -- Previously: WHERE a.status IN ('sold', 'paid', 'closed')
  ...
  payment_debits AS (
    ...
    -- Also fix this to match ledger - exclude only 'cancelled' payments
    -- Previously: WHERE ap.status IN ('paid', 'verified')
...
COMMENT ON FUNCTION get_ready_to_pay_artists IS 'Returns artists ready to be paid via Stripe OR manual payment request. Now matches ledger calculation (excludes closed art, excludes only cancelled payments).';

-- Log migration completion
INSERT INTO system_logs (service, operation, level, message, request_data)
VALUES (
    'migration',
    'fix_ready_to_pay_exclude_closed_art',
    'info',
    'Updated get_ready_to_pay_artists to exclude closed art status and match ledger calculation',
    jsonb_build_object(
        'migration_file', '20251019_fix_ready_to_pay_exclude_closed_art.sql',
        'applied_at', NOW()::text,
        'issue', 'Artists with $0 balance showing in ready to pay list',
        'example_artist', 'Gabriel Antonio (entry_id: 310249)',
        'example_data', jsonb_build_object(
            'artist_id', 'a4d4a187-9bef-4781-90d8-4aa846282d89',
            'art_paid', 'AB3040-2-5 ($30)',
            'art_closed', 'AB3056-1-1 ($25), AB3056-3-2 ($25)',
            'payment', '$30 paid',
            'ledger_balance', '$0',
            'old_ready_to_pay_balance', '$50 (WRONG - counted closed art)',
            'new_ready_to_pay_balance', '$0 (CORRECT - excludes closed art)'
        ),
        'fixes', jsonb_build_array(
            'art_sales_by_currency: Changed status filter from (sold, paid, closed) to (sold, paid)',
            'payment_debits: Changed status filter from (paid, verified) to != cancelled'
        )
    )
);
