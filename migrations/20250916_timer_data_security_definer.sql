-- Create a SECURITY DEFINER function for timer data access
-- This bypasses RLS but is tightly controlled to only return safe timer data
...
BEGIN
    ...
    -- Get active rounds (within 30 minutes)
    SELECT jsonb_agg(
        jsonb_build_object(
            'round', r.round_number,
            'closing_time', r.closing_time,
    ...
    WHERE r.event_id = event_record.id
      AND r.closing_time IS NOT NULL
      AND r.closing_time <= (now() + interval '30 minutes')
      AND r.closing_time > now();

    -- Get auction data (only closing times, no bid amounts or personal info)
    SELECT jsonb_build_object(
        'earliest', min(closing_time),
        'latest', max(closing_time),
        'count', count(*),
        'same_time', (min(closing_time) = max(closing_time))
    )
    INTO auction_data
    FROM art
...
-- Grant execute permission to anon role only
GRANT EXECUTE ON FUNCTION get_timer_data(text) TO anon;

-- Add comment for documentation
COMMENT ON FUNCTION get_timer_data(text) IS
'SECURITY DEFINER function for timer display. Only returns safe, public timer data for events within 36-hour window. No sensitive information included.';