-- Fix congratulations SMS message to use correct currency symbol instead of hardcoded format
-- The message was showing just the raw number without proper currency formatting
...
-- Add comment
COMMENT ON FUNCTION public.admin_actually_close_auction_items(text, text) IS
'FIXED: Congratulations SMS now uses correct currency symbol and tax-included amount instead of raw winning bid';