-- Fix art_media trigger column references
-- The table uses 'art_id' not 'artwork_id'
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'art_media' THEN
    -- FIXED: Use art_id for art_media (not artwork_id)
  ...
  CASE TG_TABLE_NAME
    ...
    WHEN 'art_media' THEN
      ...
      v_notification_payload := jsonb_build_object(
        ...
        'art_id', NEW.art_id,  -- FIXED: Use art_id not artwork_id
        'timestamp', EXTRACT(EPOCH FROM NOW()),
        'cache_version', v_cache_version
      );
  END CASE;
...
'Fixed art_media trigger to use correct column name: art_id instead of artwork_id.
