...
serve(async (req) => {
  ...
  try {
    ...
    if (!eventEid || eventEid === 'auction-csv-export') {
      return new Response(
        JSON.stringify({ error: 'Event EID is required in URL path (e.g., /auction-csv-export/AB3019)' }),
    ...
    const [
      { data: artworks, error: artError },
      { data: paymentLogs, error: paymentLogsError },
      { data: paymentStatuses, error: paymentStatusesError },
      { data: bidderData, error: bidderError },
      { data: directBids, error: directBidsError },
      { data: stripePayments, error: stripePaymentsError }
    ] = await Promise.all([
      ...
      supabase.rpc('get_payment_logs_admin', { p_event_id: event.id }),
      
      // Get payment statuses using our working function  
      supabase.rpc('get_payment_statuses_admin', { p_event_id: event.id }),
    ...
    const auctionData = artworks?.filter(artwork => artwork.artist_profiles?.entry_id).map(artwork => {
      ...
      return {
        ...
        buyer_first_name: bidInfo?.highestBidder?.first_name || 
                          (stripePayment?.metadata?.buyer_name?.split(' ')[0]) || '',
        buyer_last_name: bidInfo?.highestBidder?.last_name || 
                         (stripePayment?.metadata?.buyer_name?.split(' ').slice(1).join(' ')) || '',
        buyer_nickname: bidInfo?.highestBidder?.nickname || '',
        buyer_email: bidInfo?.highestBidder?.email || 
                    stripePayment?.metadata?.buyer_email || '',
        buyer_phone: bidInfo?.highestBidder?.phone_number || 
                    stripePayment?.metadata?.buyer_phone || 
                    bidInfo?.highestBidder?.auth_phone || '',
        buyer_auth_phone: bidInfo?.highestBidder?.auth_phone || 
                         stripePayment?.metadata?.buyer_phone || '',
    ...
    const csvRows = auctionData.map((item: any) => [
      item.art_code || '',
      item.round || '',
      item.easel || '',
      item.artist_name || '',
