-- Fix process_bid_secure to properly handle person records with duplicate phone numbers
-- Always prefer the person record with auth_user_id link, or link existing records
...
BEGIN
  ...
  IF v_person_id IS NULL THEN
    -- No person record linked to auth user, look for existing person with same phone
    -- Try different phone formats: with +, without +, normalized
    ...
    IF v_existing_person_id IS NOT NULL THEN
      ...
      SET
        ...
        first_name = COALESCE(first_name, v_auth_metadata->>'person_name', v_nickname),
        updated_at = NOW()
    ...
    ELSE
      ...
      INSERT INTO people (
        ...
        v_auth_phone,
        v_nickname,
        COALESCE(v_auth_metadata->>'person_name', v_nickname),
        NOW(),
        NOW()
      );
    ...
    END IF;
  ELSE
    -- Update existing person record with latest phone and metadata
    UPDATE people
    SET
      auth_phone = v_auth_phone,
      phone_number = COALESCE(phone_number, v_auth_phone),
