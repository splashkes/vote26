-- Add actual payment collection tracking to payment_logs
-- This allows tracking differences between winning bids and actual amounts collected

-- Add new columns to payment_logs table
ALTER TABLE payment_logs ADD COLUMN IF NOT EXISTS actual_amount_collected NUMERIC(10,2);
ALTER TABLE payment_logs ADD COLUMN IF NOT EXISTS actual_tax_collected NUMERIC(10,2);
ALTER TABLE payment_logs ADD COLUMN IF NOT EXISTS payment_method TEXT;
ALTER TABLE payment_logs ADD COLUMN IF NOT EXISTS collection_notes TEXT;

-- Add constraint for payment_method
ALTER TABLE payment_logs ADD CONSTRAINT payment_logs_payment_method_check 
CHECK (payment_method IS NULL OR payment_method IN ('cash', 'card', 'check', 'other', 'stripe'));

-- Update admin_update_art_status function to accept actual payment amounts
CREATE OR REPLACE FUNCTION admin_update_art_status(
  p_art_code TEXT,
  p_new_status TEXT,
  p_admin_phone TEXT DEFAULT NULL,
  p_actual_amount_collected NUMERIC DEFAULT NULL,
  p_actual_tax_collected NUMERIC DEFAULT NULL,
